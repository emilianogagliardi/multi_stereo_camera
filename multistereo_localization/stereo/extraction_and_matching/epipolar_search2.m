function [kLm, kRm, dLm, dRm] = epipolar_search2(descL, descR, kptsL, kptsR, norm_function)
...
for idxL = 1:size(kptsL, 1)    
    ...
    if kptsL(idxL, 2) < kptsR(idxR, 2) - EPIPOLAR_TICKNESS / 2
        while true
            idxL = idxL + 1;
            if idxL == size(kptsL, 1) + 1
                break;
            end
            if equal(kptsL(idxL, 2), kptsR(idxR, 2),  EPIPOLAR_TICKNESS)
                break;
            end
            if kptsL(idxL, 2) > kptsR(idxR, 2) - EPIPOLAR_TICKNESS / 2
                break;
...
function o = compare(x, i, j, precision)
    ...
    elseif smaller(x(i, 2), x(j, 2), precision)
        ...
        if greather(x(i, 1), x(j, 1), precision)
            o = 1;
